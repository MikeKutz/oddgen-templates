...
TYPE BODY template_repo_interface_v1 AS
  ...
  not final member procedure cd( p_directory in varchar2 := null ) AS
    l_stuff directory_t;
  BEGIN
    l_stuff := repo_const.extract_dir_info( p_directory );

    if l_stuff.repository != self.mount_point then raise repo_const.invalid_repository; end if;
  ...
  BEGIN
    if p_directory != '/' then raise repo_const.invalid_directory; end if;
  END check_dir;
  ...
  BEGIN
    ...
    raise repo_const.invalid_interface;
  END check_file;
